...
export default function ProductCard({ product, onAddToCart }: ProductCardProps) {
    return (
        <div className={`group relative flex flex-col items-center p-4 bg-white rounded-3xl transition-all duration-300 hover:shadow-xl hover:shadow-blue-100 hover:-translate-y-1 cursor-pointer border border-transparent hover:border-blue-50 ${product.span || ''}`}>
            {/* Image Container */}
            <div className={`relative w-full ${product.aspect || 'aspect-[4/5]'} overflow-hidden rounded-2xl bg-gray-50 mb-6`}>
                <img
                    src={product.image}
                    alt={product.name}
